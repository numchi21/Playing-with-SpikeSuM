...
import SpikeSuMC_network as SpikeSuMC_network  # noqa: E402
from run_simulation import run_simulation  # noqa: E402
warnings.filterwarnings("ignore")
# scripts/spikesum.py writes to scripts/logs/; this used <repo>/logs/, which
# is why a stray logs/ folder appeared in the repository root. Same place now,
# and created if missing instead of crashing.
LOG_DIR = os.path.join(path, 'scripts', 'logs')
os.makedirs(LOG_DIR, exist_ok=True)
os.makedirs(os.path.join(path, 'results', 'optimisation'), exist_ok=True)
outputfile = open(os.path.join(LOG_DIR, 'logs_single_run.txt'), "w")
outputfile.close()
import Bayesian_change_detection as bcd  # noqa: E402
import utils_other_alg  # noqa: E402
...
from IPython.display import clear_output  # noqa: E402
# warnings.filterwarnings("ignore")
def optimise_model(param_network, simulations, model):
    ...
    write_file = os.path.join(LOG_DIR, 'log_optimisation_{0}.txt'.format(model))
    print('---------------------------START OF OPTIMIZATION--------------------------------------------------', file=open(write_file, "w"))
    global min_error
    min_error = epochs
    # `model == 'SNN' or 'SNNrand'` was always true: a non-empty string is
    # truthy, so this branch ran for every model and only happened to work
    # because the later ifs overwrote SPACE.
    if model in ('SNN', 'SNNrand'):
        SPACE = [
    ...
    def objective(**params):
        ...
        if 'SNN' in model:
            ...
            # n_memory is 1 here, so the failure criterion never fires and
            # run_simulation cannot abort early. Guard anyway: a truncated run
            # has fewer error samples, and its mean would look artificially
            # good, which would bias the search towards parameters that die.
            recorded = net.info["error"].shape[-1]
            if recorded < simulations[0]['epochs'] - 1:
                print('Run aborted after {0}/{1} epochs, scoring as failure'.format(
                    recorded, simulations[0]['epochs']), file=open(write_file, "a"))
                return 100
...
#             utils.save(net.info, file = prefix, type_= 'data')
    ...
    try:
        ...
        # No previous run to warm-start from (missing file, or a results file
        # written by a different skopt version). Start from scratch.
...
for model in ['VarSmile']:
     ...
     for number_rooms in [16,32]:
        for n_moves in [4]:
            ...
            epochs = 5000
            batch_size = 1
            # The filename has no placeholders, so .format(n_moves, volatility) did nothing: 
            # every configuration silently loaded the same file.
            params_path = os.path.join(
                path, 'scripts', 'params',
                'params_network_n_moves_{0}_H_{1}.pkl'.format(n_moves, volatility))
            if not os.path.exists(params_path):
                print('No params file for n_moves={0}, H={1}; '
                      'falling back to params_network.pkl'.format(n_moves, volatility))
                params_path = os.path.join(path, 'scripts', 'params',
                                           'params_network.pkl')
            seeds = []
