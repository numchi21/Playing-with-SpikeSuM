...
# They decide when a simulation is declared broken

CP_DETECTION_LIMIT = 50      # epochs allowed to pass before a switch is detected
WRONG_SWITCH_LIMIT = 50      # spurious module switches tolerated
SPURIOUS_SWITCH_DELAY = 30   # a switch this long after a change point is spurious
MAZE_FAMILIARITY_LIMIT = 50  # presentations before a rule counts as 'seen before'

# With True the verdict means "the network is failing right now", which is what
# makes an early abort safe. Set to False to restore the cumulative behaviour
# that scripts/optimization.py was tuned against.
RESET_CRITERIA_ON_CHANGE_POINT = True

# How many consecutive rule episodes must be flagged before run_simulation
# aborts. 
CONSECUTIVE_FAILED_EPISODES = 3
...
class SpikeSuMC(object):
    ...
    def optimisation_stopping_criterium(self, criterium):
        ...
        """
        if (criterium['wrong_change_point'] > WRONG_SWITCH_LIMIT
                or criterium['before_detecting_cp'] > CP_DETECTION_LIMIT) \
                and self.n_memory > 1:
            criterium['stop'] = True
            return 1
    ...
    def reset_criteria_after_change_point(self, criterium):
        """Clear the failure counters when a new rule starts.

        Called from run_simulation.pre_step alongside the resets that were
        already there. Without this, 'wrong_change_point' and 'stop' keep the
        verdict of an episode that is already over.
        """
        # Count how many rule episodes in a row ended flagged as failed.
        if criterium['stop']:
            criterium['failed_episodes'] = criterium.get('failed_episodes', 0) + 1
        else:
            criterium['failed_episodes'] = 0

        criterium['last_change_point'] = 0
        criterium['before_detecting_cp'] = 0
        criterium['cp_detected'] = False
        if RESET_CRITERIA_ON_CHANGE_POINT:
            criterium['wrong_change_point'] = 0
            criterium['stop'] = False
    ...
    def step_info(
        ...
        # Evenly spaced plot points instead of [1, 500, 1500, 4000, ...], which
        # only made sense for a 10000-epoch run.
        epochs = simulation["epochs"]
        plot_times = {1, epochs - 1} | {i * epochs // 5 for i in range(1, 5)}
        ...
        if maze not in criteria[b]['observed_mazes']:
                ...
                if criteria[b]['maze_{0}'.format(maze_n)] > MAZE_FAMILIARITY_LIMIT:
        ...
        if old_memory != estimated_active_memory and epoch > 1:
            ...
            if not criteria[b]['cp_detected'] or epoch < 10:
            ...
            if criteria[b]['last_change_point'] > SPURIOUS_SWITCH_DELAY:
