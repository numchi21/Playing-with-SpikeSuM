...
# gammaln and DIR below referenced sc_gamma and digamma without importing them,
# so any call raised NameError. They are duplicates of the numpy versions in
# utils_other_alg.py and nothing in this module calls them, but they are kept
# working rather than deleted in case a notebook imports them.
def gammaln(a):
...
def draw_Dirichlet_maze(len_maze, s):
    ...
    """
    # torch.random.dirichlet does not exist; the module only holds RNG state
    # helpers. This branch raised AttributeError whenever create_simulation was
    # called with Dirichlet != 0. torch.distributions is the real API.
    concentration = s * torch.ones(len_maze)
    return torch.distributions.Dirichlet(concentration).sample((len_maze,))
...
def create_simulation(
    ...
    """
    if seed:
        # Seed both generators: the mazes below are drawn with torch, but the
        # room sequence is re-seeded further down with random.randint(), which
        # comes from Python's own RNG.
    ...
    maze_presentations = {}
    # Re-seeding here decorrelates the room sequence from the maze layouts, so
    # two simulations with different seeds do not walk the same path. 
    # Previously Python's RNG was never seeded, so the change points differed on
    # every call even with a fixed seed, and they had to be copied by hand from
    # the terminal to be recovered afterwards.
